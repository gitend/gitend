...
打开会返回会话 id 与有界启动消息。pwsh 启动会返回跨越所有启动等待所保留的 scrollback，受配置的 scrollback 与读取上限约束，即使最后一次就绪等待没有输出也不例外。发送以四种等待原因之一与一个会话状态结算；`session_exit` 表示顶层 shell 已退出。设置失败会拒绝打开：受限模式下缺少沙箱提供方、shell 在启动期间退出、shell 未能在启动超时前达到就绪，或调用方取消。清理失败会拒绝关闭，而不是声称成功。
