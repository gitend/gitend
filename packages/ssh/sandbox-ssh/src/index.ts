...
const factsSchema = z.object({ argv: z.array(z.string()), enforcement: z.enum(['full', 'partial']), denialSignatures: z.array(z.string()), runnerFailureRules: z.array(z.object({ allowedExitCodes: z.array(z.number().int()).optional(), fatalSignatures: z.array(z.string()), informationalLines: z.array(z.string()).optional() }).strict()) }).strict()
...
export class SshSandboxProvider extends SandboxProvider {
  ...
  override confine(argv: readonly string[], policy: SandboxPolicy): ConfinedArgv {
    ...
    return {
      argv: [this.node, this.ctx.ssh.helperPath, '--confine', JSON.stringify(policy), '--', ...argv],
