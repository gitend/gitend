...
/**
 * Read activation diagnostics after a runtime update.
 * @param ctx - the settled Loader context.
 * @returns diagnostics for inactive entries.
 */
export async function recordContainedStates(ctx: Context): Promise<string[]> {
  return (await inactiveEntries(ctx)).map(failure => failure.diagnostic)
}
