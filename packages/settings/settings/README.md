...
- **Two user layers, no record of each field’s supplying layer** — resolution knows schema defaults, one composition `base` per instance, the global section, and one scoped section; a descriptor's `user` and `inherited` let a surface tell an override from an inherited value, but the service does not record which layer supplied each resolved field.
