...
无 payload 的 `permission-presets/catalog-changed` 事件通过该 source 转发。Client 在首次读取 `permissionPresets.catalog()` 前先订阅，每次通知后重新读取完整快照；该事件只表达失效，不复制目录状态，也不改变 Session sequence。
