...
The payload-free `permission-presets/catalog-changed` event is forwarded through this source. A Client subscribes before its first `permissionPresets.catalog()` read and re-reads the complete snapshot after each notification; the event carries invalidation only and never duplicates catalog state or changes a Session sequence.
