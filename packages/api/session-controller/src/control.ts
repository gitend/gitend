...
import type {
  JsonValue, Session, SessionEvent, SessionId, UserMessage,
...
function queueItems(agent: Agent): SessionQueuedItem[] {
  // The inbox projection folds the committed splice before `session/event`
  // reaches this controller (the registry subscribes earlier), so the live
  // lists already reflect the mutation that triggered the broadcast.
