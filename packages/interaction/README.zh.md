---
description: "人机协作与授权能力族的包映射：斜杠命令、一次性审批、权限预设、逐调用 Auto review，以及让运行中的 agent 暂停等待人类决定的问题。"
kind: "package-group"
---
...
`interaction/` 组承载人机协作，也让部署可以授权单次工具调用。它提供斜杠命令、一次性审批决定、具名权限预设、shipped Web 的实验性同模型 Auto review，以及 agent 需要人类决定时暂停等待的问答服务。六个包都是产品包——由用户直接操作或负责执行所选交互策略的真实接口——产品 `dsh` CLI 直接组合它们。交互式应用直接驱动命令、审批、权限与提问接口，自动化则改用 ACP 传输。子系统参考与包 README 拥有穷尽式约定；本映射指向每个包及其相邻包。
...
| [`auto-review/`](auto-review/README.zh.md) | 在执行前使用当前提供方／模型审查 Auto 模式的每次原生或 PTC inner tool call | 注册到 `ctx.permissionPresets` 与 `ctx.tools` |
...
- [Auto review 包](auto-review/README.zh.md)——五分区 reviewer 请求、fail-closed 决定与生命周期。
