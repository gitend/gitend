...
Publication and teardown are fail-closed. A persisted Auto session cannot publish without the live Auto registration. Disposal closes new admission and switches live Auto sessions through the existing preset writer to `danger-full-access`, preserving the shared sandbox/approval bundle and persistent terminals. The plugin then aborts its lifecycle and waits for every active review or admitted call before removing the listener and Auto registration. An allowed call remains active until `tools/execute` or `tools/result`; the execute wrapper combines the lifecycle signal with the caller signal before delegating, so disposal cancels any admitted body that has not started. Cancellation and draining also run if a preset write fails; Cordis reports the cleanup error and retains the closed registrations, which reject new Auto selections and calls from sessions still in Auto. Reinstalling the integration restores only the catalog entry; migrated live sessions remain in Full access until a user selects Auto again.
