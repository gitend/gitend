...
`apply` must be synchronous and must return the same state reference for events that do not concern the unit — an unchanged reference means zero downstream work. Every served projection value is a complete read model. Source events may carry complete values or domain-owned operations; deterministic `apply` replay, including checkpoint plus forward-tail replay, must reconstruct the same state.
