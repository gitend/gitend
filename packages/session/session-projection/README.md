...
- **Whole-value projection rule.** Every served projection value is a complete read model. Source events may carry whole values or domain-owned operations; the unit's deterministic `apply` owns replay, while checkpoint plus forward tail replay produces the same state without repeating aggregates in the log.
