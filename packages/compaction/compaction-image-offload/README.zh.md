...
`dsh-compaction-image-offload` 让图片密集的会话保持可发送。支持图片的路由以 `IMAGE_OFFLOAD_REQUIRED` 让请求失败并说明最老的几张保留图片已经装不进预算时，本插件把承载这些图片的每个表层节点替换为图片块标了 `offloaded` 的副本，通过 compaction seam 的 `compaction/prune` 协议为被替换节点计价，然后重试该 step。此后每条路由都以注明图片及其只读路径的占位文本代替图片发送。和其他 compaction 执行器一样，替换是持久的表层事实，永不回退，因此模型看到的图片和 provider 缓存前缀只向前移动。
