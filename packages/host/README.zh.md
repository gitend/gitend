...
`host/` 组提供 Web GUI 的普通 HTTP 服务器、服务已构建 Web 壳的 SPA dist 服务器、带原生／浏览／自适应组合包的工作区目录选择 seam、只读的插件清单投影，以及共享插件管理器之上的 `plugins` Remote。这八个包都是产品包；浏览器传输位于 [`client/`](../client/README.zh.md)，组合应用是 [`apps/cli`](../../apps/cli/README.zh.md)，它启动 [`dsh-base` 组合包](../bundle/base/cordis.patch.yml) 来提供 `apps/web/` 下的 Web 应用。选择器后端可在共享 seam 后互相替换。
...
| [`plugin-manager/`](plugin-manager/README.zh.md) | `plugins` Remote：转接到共享插件管理器，并把其失败映射为 Remote 错误码 | `ctx.pluginManager`、Remote `plugins/*` |
