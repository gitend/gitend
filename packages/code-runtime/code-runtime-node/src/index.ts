...
import type { CodeBindingNamespace, CodeJsonValue, CodeRunFailure, CodeRunRequest, CodeRunResult, CodeRunSandbox, CodeRunSpec } from '@deepseek-ai/dsh-code-runtime'
...
export class NodeCodeRuntime extends CodeRuntime {
  ...
  constructor(ctx: Context, config: Config) {
    ...
    for (const [key, value] of Object.entries(this.config)) {
      if (typeof value === 'number' && (!Number.isFinite(value) || value <= 0)) throw new Error(`code-runtime-node: ${key} must be positive and finite`)
    }
    for (const key of ['timeoutMs', 'maxTimeoutMs', 'graceMs'] as const) {
      if (this.config[key] > MAX_TIMER_DELAY_MS) throw new Error(`code-runtime-node: ${key} exceeds the supported timer range`)
    }
    if (!Number.isSafeInteger(this.config.maxOutputBytes) || this.config.maxOutputBytes < 4) throw new Error('code-runtime-node: maxOutputBytes must be an integer of at least 4')
    if (!Number.isSafeInteger(this.config.maxMessageBytes) || this.config.maxMessageBytes > 0xffff_ffff) throw new Error('code-runtime-node: maxMessageBytes must fit an unsigned 32-bit frame length')
    if (!Number.isSafeInteger(this.config.maxPendingCalls)) throw new Error('code-runtime-node: maxPendingCalls must be an integer')
    if (!Number.isSafeInteger(this.config.maxOldGenerationSizeMb)) throw new Error('code-runtime-node: maxOldGenerationSizeMb must be an integer')
    if (this.config.nodeExecutable.length === 0) throw new Error('code-runtime-node: nodeExecutable must be non-empty')
    if (this.config.bootstrapPath !== undefined && !isAbsolute(this.config.bootstrapPath)) throw new Error('code-runtime-node: bootstrapPath must be absolute')
    ctx.effect(() => async () => {
      this.disposed = true
  ...
  resolve(request: CodeRunRequest): CodeRunSpec {
    ...
    return {
      ...request,
      cwd,
      timeoutMs: clampTimeout(request.timeoutMs, this.config.timeoutMs, this.config.maxTimeoutMs, 'code-runtime-node: timeoutMs'),
      sandboxPolicy,
    }
  }
  ...
  async run(spec: CodeRunSpec): Promise<CodeRunResult> {
    if (this.disposed) throw new Error('code-runtime-node: run after disposal')
    if (spec.sandboxPolicy === undefined) throw new Error('code-runtime-node: run requires a resolved sandbox policy')
    if (!isAbsolute(spec.cwd) || !Number.isFinite(spec.timeoutMs) || spec.timeoutMs <= 0 || spec.timeoutMs > this.config.maxTimeoutMs) throw new Error('code-runtime-node: run requires resolved cwd and timeout')
  ...
  private async execute(
    ...
    const logs: string[] = []
    const sandbox: CodeRunSandbox = { mode: policy.mode, denied: false }
    const result = Promise.withResolvers<CodeRunResult>()
    ...
    const finish = (failure?: CodeRunFailure, value?: CodeJsonValue): void => {
      if (settled) return
      settled = true
        ...
        switch (raw.type) {
          ...
          case 'done': {
            if (raw.error !== undefined) {
              ...
              const failure = { kind: raw.error.kind, message: raw.error.message } as CodeRunFailure
            ...
            const value = raw.value === undefined ? undefined : decodeCodeJsonWire(raw.value)
          ...
          case 'call': {
            ...
            void (async () => {
              ...
              try {
                ...
                reply = { type: 'reply', id, ok: true, value: encodeCodeJsonWire(value) }
              } catch (error: unknown) {
