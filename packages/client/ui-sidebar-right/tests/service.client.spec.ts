...
function harness() {
  ...
  const instance = createSidebarRightStore(() => ({ kind: 'guide', title: 'seed', permanent: false })).create()
...
describe('SidebarRightController — a tab\'s own actions', () => {
  ...
  it('land in the session the tab is in through its own adopted store, after another session\'s seat took over', () => {
    ...
    const other = createSidebarRightStore(() => ({ kind: 'guide', title: 'seed', permanent: false })).create(OTHER)
  ...
  it('adoption syncs the Tab domain on each commit of that store: the seeded guide is pinned, a closed tab aborted', () => {
    ...
    const replacement = createSidebarRightStore(() => ({ kind: 'guide', title: 'seed', permanent: false })).create()
