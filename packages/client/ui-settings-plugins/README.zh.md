...
使用**插件**设置分区可以配置当前部署公开的插件，也可以打开功能插件经 `settings.plugins.tab` 贡献的页面。**插件配置**标签页会为每个受支持的插件展示一张可展开卡片，标明用户覆盖过哪些值，并允许用户将它们重置为部署默认值；同样的卡片构成每个预设详情页的**设置**段，在那里编辑该预设自己的值并标注哪些是继承来的。卡片会在本地保留修改，直到用户保存。如果配置在卡片加载后发生变化，保存会被拒绝，而不会覆盖较新的值。
...
预设的详情页——Agent 预设分区里卡片上的齿轮之后——带一个**设置**段，用同样的卡片编辑该预设的命名作用域（`preset/<id>`）。在那里，预设未覆盖的字段在共用用户层持有它时显示**继承**，重置暂存的是继承值而非组装默认值。保存的值写进该预设自己的段：由该预设组装的运行中会话立即读到，之后的会话在启动时读取。草稿属于输入时所在的作用域，离开页面后仍在。配置标签页始终编辑共用值。
