...
打开设置中的「插件」分区并选择**插件配置**标签页，即可编辑本部署所组装的宿主平面插件。卡片依次为 shell 执行器（`bash`）、agent loop（智能体循环）的工具调用并行度（`agent-loop`）、Subagent 委派限制与模型选择（`subagent` 和 `subagent-model-selection`）以及 DeepSeek 搜索提供方（`web-search-deepseek`）。
...
保存 Subagent 卡片时会校验两个分区，并通过各自原有命名空间写入草稿。这些写入相互独立：若其中一项失败，卡片保持展开并保留该草稿，重试仅写入剩余草稿。若部署只提供模型选择而不提供限制设置，模型命名空间的注册项会呈现同一张卡片。
