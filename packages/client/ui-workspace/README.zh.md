...
**最近更新**在分组和单列表视图中都按普通 Session 的最近一次用户提示词或 steering（中途引导）时间降序排列。当前选中的空白**新会话**在首条提示词落地前保持首位。**手动排序**保留当前顺序，并在活动变化时保持已有位置。两种模式都把新建空白 Session 放在首位；手动排序会在首条提示词落地和重新加载后保留拖拽位置。返回最近更新会丢弃手动位置并按时间排列。浏览器默认采用最近更新，并在重新加载后记住所选模式。拖拽 Session 会在可见顺序上应用移动、保存结果并选中手动排序；真实 Workspace 的拖拽还会更新 Host Session 记账，而 Ungrouped 和单列表的顺序保留在浏览器本地。折叠分组的拖拽边界按渲染行确定，并把来源行放在中间隐藏行之前，因此拖拽不会隐藏来源行。无论采用哪种 Session 顺序，Workspace 拖拽顺序都由 Host 持久化。
...
Workspace 列表基线就绪后，浏览器持久化的展开状态和手动 Session 顺序记录只保留当前 Workspace id、Ungrouped 和单列表记账。手动顺序中，真实 Workspace 从 `WorkspaceView.sessionIds` 初始化，Ungrouped 和单列表从最近更新时间顺序初始化。最近更新直接从每份当前列表快照派生，不读取或写入这些已保存位置；时间相同时按 Session id 稳定排序。共享侧边栏投影会隐藏持久化 Session 摘要中带有 `origin: 'subagent'` 的行；每个可见普通行都会在经不间断的 subagent 谱系可达的任一后代运行时继承蓝色活动指示器。同一项纯派生逻辑还会为分组、平铺与搜索节点读取列表 projection value 中的 Schedule key；本包只使用纯类型依赖 `@deepseek-ai/dsh-schedule/client`，不会导入 Schedule 运行时或 `ui-schedule`。
