...
**最近更新**在分组和单列表视图中都按普通 Session 的最近一次用户提示词或 steering（中途引导）时间降序排列。当前选中的空白**新会话**在首条提示词落地前保持首位。**手动排序**恢复保存的顺序，并在活动变化时保持已有位置。拖拽 Session 会在可见顺序上应用移动、保存结果并选中手动排序；真实 Workspace 的拖拽还会更新 Host Session 记账，而 Ungrouped 和单列表的顺序保留在浏览器本地。折叠分组的拖拽边界按渲染行确定，并把来源行放在中间隐藏行之前，因此拖拽不会隐藏来源行。无论采用哪种 Session 顺序，Workspace 拖拽顺序都由 Host 持久化。
