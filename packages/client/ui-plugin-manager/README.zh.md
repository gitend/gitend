...
组合包开关改变其层选择。实时 profile 在操作完成前重组；下次启动才应用 patch 的 profile，以及被更高层覆盖的组合包，会以 toast 说明。Host 读不了的组合包带异常标签，其页面给出原因，且不能打开；提供管理组件的组合包带着 Host 的原因保持锁定。随安装提供的组合包不在本页；设置中「插件」分区的「插件列表」标签页负责查看它们。

### 切换组合包里的一行

组合包页面上行的开关调用 `pluginManager.setPluginEnabled`，往 profile 的 `cordis.patch.yml` 写入该行的 `disabled` 覆盖。实时 profile 的树随即重组，该行的宿主半区卸下或挂上，组合包其余部分照常运行；组合包自己的客户端 bundle 挂载的浏览器半区要到页面重载才消失。行按 Host 运行它们的 fiber 阶段显示状态。开关只出现在已打开的组合包上；没有存活条目的行，以及 Host 不通过 profile patch 寻址的行，带着 Host 的原因锁定。组合包 patch 没有声明却改动的行，作为它改动的内置组件列出。超过十行的列表带一个按行 id 筛选的输入框。
...
- **只管理组合包**——没有组合包 patch 的依赖在安装前就被拒绝，也从不列出；加载普通插件模块仍是文件操作。
