...
该标签页首次选择时读取宿主清单，插件激活期间不执行 Remote 调用。提供[插件管理器](../../boot/plugin-manager/README.zh.md)的宿主还支持组合包安装、删除及可唯一定位的全局条目开关。操作后刷新实际状态，并显示失败、覆盖与待重启结果；清单刷新成功时仍保留最近一次操作错误；预设组合保持只读。
...
管理错误码和只读原因使用当前语言的词典。页面在本地化状态旁显示原始包管理与 Loader 诊断、清理结果和残留依赖。无效的 profile 依赖仍可删除，但启停开关不可用。
