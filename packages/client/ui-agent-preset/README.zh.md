...
使用本包可以为新的 Web GUI 会话选择 agent preset、在会话标题中查看当前 preset，并在设置中管理可用 preset——复制、删除、设为默认、打开 preset 自己的文件，以及打开每个 preset 的详情页，其他插件经 `settings.agentPreset.detail` 槽位把自己知道的内容加到那里。preset 在会话创建时即固定，因此更改选择或默认值只影响此后创建的会话。如果部署未提供任何 preset，这些控件保持隐藏，每个会话都使用宿主组装。
