...
Use this package to choose the agent preset for a new Web GUI session, see its active preset, and manage presets in Settings. Copy or delete presets, choose a default, open their files, or open their detail pages. The Agent mode picker is shown by default and can be hidden without changing running or historical sessions. Presets are fixed when sessions are created; choices apply to new sessions. Without a preset provider, these controls stay hidden and every session uses the host composition.
