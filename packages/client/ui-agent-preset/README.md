...
Use this package to choose the agent preset for a new Web GUI session, see the active preset in the session header, and manage available presets in Settings — copy, delete, set the default, open a preset's own files, and open each preset's detail page, where other plugins add what they know about it through the `settings.agentPreset.detail` slot. A preset is fixed when a session is created, so changing the selection or default affects only later sessions. If the deployment provides no presets, these controls stay hidden and every session uses the host composition.
