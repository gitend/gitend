  ...
  const requestNumbers = useMemo<readonly TrajectoryRequestNumber[]>(() => {
    ...
    for (const [index, entry] of orderedRequests.entries()) {
      ...
      if (entry.request?.purpose !== 'compaction') {
        ...
        const provider = request?.providerInfo?.provider ?? node?.providerInfo?.provider
        const model = request?.providerInfo?.model ?? node?.providerInfo?.model
      ...
      numbered.push({
        ...
        ...(request.providerInfo?.provider === undefined
          ? {}
          : { provider: request.providerInfo.provider }),
        ...(request.providerInfo?.model === undefined
          ? {}
          : { model: request.providerInfo.model }),
