...
`suppressRuntimeContext()` rejects assembly when an effective context is required; otherwise it removes every dynamic runtime-context contribution for the calling scope without disabling the services that own the underlying facts; multiple suppressors compose and the effect restores context when none remains.
