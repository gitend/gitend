...
describe('remote proxies over a booted client', () => {
  it('boots the Office Remote subset from source and routes calls through its mock', async () => {
    const name = '@deepseek-ai/dsh-api-document-render-controller'
    const mock = RemoteMock.create().load(remoteDefaultResponses).unary('documentRender/render', ok({ bytes: 3 }))
    ...
    await expect(remote.documentRender!.render!({ path: 'report.docx' })).resolves.toEqual(ok({ bytes: 3 }))
    expect(mock.log.calls('documentRender/render').map(call => call.args)).toEqual([[{ path: 'report.docx' }]])
  })
