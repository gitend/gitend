...
外部插件使用正常的 Node 包解析。Desktop 不递归检查 peer 版本、重复包、链接包或祖先目录依赖解析。这些检查重复了包管理器和加载器的职责，并拒绝 pnpm 支持的安装来源。共享宿主链接仍然可用，但插件可以解析到另一个已安装副本；不兼容插件可能在 Host 启动时失败，需要通过独立的桌面壳 UI 恢复。如果第三方包需要宿主范围的实例身份，必须明确加入运行时共享清单；版本号相同并不足够。
