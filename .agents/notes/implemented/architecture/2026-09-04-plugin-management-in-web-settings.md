...
**The page speaks two nouns and four states.** A person who does not know the Loader sees *plugin packs* (switched as a whole, serving every session) and *plugins* (joining a preset or every session through **Add to…**), each card with a name, a one-liner, one of four states — running, off, restart needed, problem — and its uninstall beside the switch. Built-in packs fold behind one line. Harness modules in a preset read by the tab's own dictionary (`name.<row id or module slug>` / `desc.<…>`), third-party ones by their manifest title and description with a third-party mark. Entry ids, module names, cordis copies, probe times, and overridden rows stay off the page; the read-only **Plugin list** tab is gone with its package, `ui-settings-plugin-inventory`, while the `pluginInventory` Remote stays as the manager's source of preset compositions and host-tree module names.
