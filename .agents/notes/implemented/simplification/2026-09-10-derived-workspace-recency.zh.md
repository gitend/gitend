...
只有手动排序使用浏览器持久化顺序。拖拽 Session 会保存移动后的可见排列，并原子地选中手动排序。真实 Workspace 的拖拽还会调用既有的 Host 重排序操作；Workspace 分组顺序仍由 Host 拥有。进入手动排序会固定当前的时间顺序。返回最近更新会丢弃手动布局；再次进入手动排序时，从当前时间戳确定的顺序开始。所有记账都从最近更新时间顺序初始化。新出现的空白行放在首位；其保存的手动位置在首条提示词落地和重新加载后保留。
...
手动排序暂停当前列表的自动排列。默认模式为最近更新，重新加载保留所选模式和当前手动位置。按时间浏览放弃持久化的拖拽例外。视图存储保留手动顺序的成员对齐和空白行创建时的位置处理，但不拥有活动时间戳。

已归档的侧边栏记录保持为冻结的历史证据；其折叠和 Workspace 顺序决策不在本次修改范围内。没有活动记录拥有被替代的共享顺序策略。

## 测试

组件回归覆盖较旧记录首次到达、时间修正和回退、已丢弃布局的隔离、模式往返切换、空白行插入、重新加载，以及拖拽后自动选中手动排序。录制会话 Web 场景在实际发布组合中验证旧的持久化位置、原生拖拽、分组与单列表视图及重新加载。既有折叠用例保留临时空白行配额。
