...
文档只读。待处理审批显示状态、侧边栏入口和决定按钮；完整正文自动在侧边栏打开。临时的会话级 store 记录每个已自动打开的调用，让审批组件重新挂载时仍保留用户的关闭选择。历史卡片不会触发自动导航。卡片采用文件产物样式，包含 Markdown 图标、计划标题和“打开”操作。审批仍由待处理提问的属主负责；重新打开历史文档不会恢复已结束的审批，也不会授权实施。即使标题相同，不同提交仍彼此独立。
