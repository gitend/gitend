...
- The composer chip (`PermissionSelect`, ui-permission-presets) is contributed through ui-conversation's session-scoped `conversation.input.permission` slot and intercepts a Full-access pick before submission: `confirmation`/`acknowledged` component state opens the dialog, confirm calls the injected `select('danger-full-access')` callback backed by the existing `/permission` writer, and cancel/Escape/close/mask leave the current preset untouched with the checkbox reset. The confirmation revokes itself when the session locks or either input disappears, and the session-scoped slot resets with its Session. Copy rides the permission package's `permission.access` locale namespace as `confirm.*` keys.
- The `/permission` popup (ui-permission-presets over the ui-commands shell) gates through data, not a second dialog implementation: `SelectOption` carries an optional `confirmation` payload, the popup controller owns the `confirming`/`acknowledged` state transitions, and `PopupSelectView` swaps the picker card for the same `RiskConfirmation` while a gated option is pending.
...
**One shared locale namespace for every surface's safety copy.** Rejected: the composer and `/permission` popup share current-session `permission.access` copy inside ui-permission-presets, while the Settings warning names a different future-session lifetime under `settings.permission`. The package keeps those dictionaries separate instead of erasing the lifetime distinction.
...
Every visible GUI path into Full access requires a deliberate, informed acknowledgement, at the cost of one extra dialog step for users who genuinely want the preset. New pickers reuse the shared dialog through their owning state machine or attach a `confirmation` payload to the popup path. Acceptance: the composer flow in `permission-select.client.spec.tsx`, the popup integration in `browser-plugin.client.spec.ts` plus the shell gates in `popup-view.client.spec.tsx` and `popup.client.spec.ts`, the default-setting gate in `permission-presets-row.client.spec.tsx`, the Modal/RiskConfirmation contract in `atoms.client.spec.tsx`, and the assembled Web replays.
