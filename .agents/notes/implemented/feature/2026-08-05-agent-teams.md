...
The tool plugin appends a durable user-role `<system-reminder>` before a teammate's first task, stating only `You are teammate "<name>".`. Shared system policy and all tool schemas stay uniform across members; execution owns role restrictions. Team tools resolve the caller's Team and accept member names, so the model needs no Team id. Ordinary Leads receive no identity message. An ordinary fork with an inherited teammate reminder receives `You are the Team Lead.`; the inherited log identifies this case even when a summary shadows the reminder. The pre-step listener reuses the latest retained reminder and restores it after compaction removes it, including after cold recovery. It preserves rejected or emptied steps and does not depend on optional runtime-context assembly. Putting identity in the system prompt changes the prefix before inherited history; trusting a one-time reminder alone leaves its survival to summarization. Existing system-embedded identities may require a one-time prompt reconciliation; retained event generations are unchanged.
