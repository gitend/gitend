...
Pwsh startup can span multiple sends before `stdin_read` confirms readiness. Its `motd` comes from the session's bounded scrollback: a final send with no output must not erase an earlier prompt. Concatenating per-send viewports would miss output between sends and require another bounded accumulator. The existing scrollback retains that output under `scrollbackLines`, `scrollbackMaxBytes`, and `maxReadBytes`.
...
- Controlled foreground evidence in the backend tests holds readiness beyond the first silence settlement, then confirms stdin readiness without new output; the returned startup message preserves the prompt and UTF-8 byte limit. The [headless PTY snapshot](../../../../snapshots/session/pty-tools-sandbox-backend/snapshot.yml) exercises the production pwsh startup coordinator with those same two settlements and pins the prompt in the model-visible open result.
