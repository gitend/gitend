...
pwsh 启动可能跨越多次 send，直到 `stdin_read` 确认就绪。其 `motd` 来自会话的有界 scrollback：最后一次 send 没有输出，不能抹去此前的提示符。拼接各次 send 的 viewport 会遗漏 send 之间的输出，还需要另一个有界累加器。现有 scrollback 在 `scrollbackLines`、`scrollbackMaxBytes` 和 `maxReadBytes` 的约束下保留这些输出。
...
- 后端测试通过可控的前台证据，将就绪推迟到第一次静默结算之后，再在没有新输出时确认 stdin 就绪；返回的启动消息保留提示符并遵守 UTF-8 字节上限。[headless PTY 快照](../../../../snapshots/session/pty-tools-sandbox-backend/snapshot.yml) 以相同的两次结算驱动生产 pwsh 启动协调逻辑，并在模型可见的打开结果中固定提示符。
