...
One Subagent card groups delegation limits and model selection, with one disclosure and save/discard footer. Both existing controllers retain ownership of their drafts, validation and namespace writes. The card blocks saving if either draft is invalid or conflicted, disables both sections during saving, and collapses only after both settle successfully. A failed section retains its draft for retry. Limit explanations stay behind information buttons beside the field labels, with concrete depth examples and counting rules. Validation errors remain visible so disclosure does not hide a blocked save.

Both namespace keys register the shared component. The model registration renders only when the limits namespace is absent, preserving model-only deployments without teaching the generic configurable tab about Subagent grouping.
...
Focused component and controller tests cover shared validation, save/discard, pending writes, partial failure and the model-only fallback. The assembled Web scenario saves both sections through one button and reads back the settings document.
