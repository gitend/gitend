...
export async function resolveBlameAuthors(measurement, repository, api) {
  ...
  for (let offset = 0; offset < entries.length; offset += 50) {
    ...
    for (const [index, [, count]] of batch.entries()) {
      ...
      if (typeof login !== 'string' || !/^[A-Za-z0-9-]+(?:\[bot\])?$/u.test(login)) {
...
export async function productionOwnership(pull, api) {
  ...
  if (current.head?.sha !== pull.headSha || !SHA.test(current.base?.sha ?? '')) {
    throw new Error('pull request changed or has no valid base SHA')
  }
  ...
  await exec('git', [
    ...
    `${server}/${pull.repository}.git`, current.base.sha, `refs/pull/${pull.number}/head`,
  ], { env: environment, maxBuffer: 16 * 1024 * 1024 })
  const { stdout } = await exec('python3', [
    fileURLToPath(new URL('blame-production.py', import.meta.url)), current.base.sha, pull.headSha,
