...
来源为 `dsh-v0.1.3-alpha.1` 祖先历史中首次 V2 写入器变更之前的最后一个 V1 源码树，关联于 PR #3349（分支 `worktree/session-format-03-v0-v1-migration`）。其源码变更日期为 2026-09-02，标题为 `fix(session): close live assistant framing races`。这标识中间源码树，而非 PR 最终合并的源码树。[预发布归档](../persistence-changes/releases/README.zh.md)没有 V1 tag，本记录不会为此虚构产品发布。
...
[完整机器目录](v1.schema.json)包含 54 个根类型和 415 种可达类型。摘要包含所有引用字段；源码名称和路径描述所选的历史源码树。
...
[仅使用源码的提取器](../../scripts/persistence-schema.ts)以 TypeScript 6.0.3 运行。重建时省略冗余的 `object` 交叉类型成员，并丢弃规范化后不可达的类型定义；保留已有的 `any` / `unknown`，不以不透明类型替代未解析的导入。
