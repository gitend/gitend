---
description: "完整的 Session V0 持久化类型、来源证据与物理编码参考。"
...
查阅 `dsh-v0.1.2-rc.1` 对应的 V0 持久化类型。Assistant chunk 是独立事件，物理 header 以可选的 `seedLength` 存储继承事件数。完整 schema 保留该检查点声明的类型，包括宽泛的数值 header 版本。
...
来源为已发布 tag `dsh-v0.1.2-rc.1`，其写入器常量为 0。这是[已收录预发布归档](../persistence-changes/releases/README.zh.md)中最后一个 V0 tag；[tag 记录](../persistence-changes/releases/dsh-v0.1.2-rc.1.zh.md)保留其发布背景。更早的 V0 tag 还存在其他类型差异，本快照不替代那些记录。
...
- `SessionHeader.version` 与物理 `JsonlHeaderLine.version` 声明为 `number`。V0 由源码写入器常量确定；仅凭数值类型不能将值限制为 0。
- 逻辑 header 要求 `isSeeded`。物理 header 则存储可选的 `seedLength`，并要求 `delegationDepth` 和 `type: "session"`。
- `assistant/chunk.data` 包含 `chunk`、`turn` 和 `step`。`assistant/message.data` 没有内嵌的 `stream`，且不存在 `assistant/attempt`。

[V0/V1 codec](../../packages/session/session-format-v0-to-v1/src/codec.ts)负责打包的 Assistant 行与范围编码的来源引用。这些物理 body 编码不是本声明类型目录中的额外根类型。[相邻迁移参考](../../packages/session/session-format-v0-to-v1/README.zh.md)负责已接受的历史规范化与拒绝规则。

打包的物理行使用 `text-chunks`、`reasoning-chunks` 或 `tool-call-chunks`，包含 `seq0`、`time0` 和 `data.turn` / `step` / `index` / `dt`。文本与推理行携带 `texts`；工具调用行携带 `id`、可选的 `name` 和 `args`。N 个成员对应 N−1 个 `dt` 时间间隔，间隔可以为负数。第 k 个成员的序号为 `seq0 + k`，时间为 `time0` 加上前 k 个间隔。物理 `sourceEventSeqs` 可以使用范围编码；下方逻辑 schema 展示展开后的数值数组。

<a id="declaration"></a>
## 机器声明

[Schema 目录](v0.schema.json)。
...
[完整机器目录](v0.schema.json)包含 54 个根类型和 415 种可达类型。摘要包含所有引用字段；源码名称和位置描述所选的历史源码树。
...
来源：`packages/core/session/src/types.ts:435`
...
来源：`packages/attachment/attachment/src/types.ts:28`
...
来源：`packages/core/session/src/types.ts:182` · `packages/interaction/commands/src/types.ts:67` · `packages/interaction/commands/src/types.ts:71` · `packages/llm/llm/src/message.ts:103` · `packages/session/session-title/src/types.ts:35`
...
来源：`packages/api/session-controller/src/types.ts:367`
...
来源：`packages/context/session-reference/src/types.ts:18`
...
来源：`packages/skill/tool-skill/src/index.ts:40`
...
来源：`packages/webhook/webhook/src/types.ts:74`
...
| 属性 | 存在性 | 类型 |
|---|---|---|
| `data` | 必需 | [`event:assistant/chunk[0].data`](#persistence-type-eventassistantchunk0data) |
| `ignorable` | 可选 | `true` |
| `seq` | 必需 | `number` |
| `time` | 必需 | `number` |
| `type` | 必需 | `"assistant/chunk"` |
...
来源：`packages/llm/llm/src/types.ts:365`
...
来源：`packages/llm/llm/src/types.ts:366`
...
来源：`packages/llm/llm/src/types.ts:367`
...
来源：`packages/llm/llm/src/types.ts:368`
...
来源：`packages/llm/llm/src/types.ts:369`
...
来源：`packages/llm/llm/src/types.ts:370`
...
来源：`packages/llm/llm/src/types.ts:371`
...
来源：`packages/llm/llm/src/types.ts:120`
...
来源：`packages/llm/llm/src/types.ts:121`
...
来源：`packages/core/session/src/types.ts:206` · `packages/llm/llm/src/types.ts:119`
...
来源：`packages/llm/llm/src/types.ts:117`
...
来源：`packages/llm/llm/src/types.ts:118`
...
| `ignorable` | 可选 | `true` |
| `seq` | 必需 | `number` |
| `sourceEventSeqs` | 可选 | [`SessionEventEnvelope[46].sourceEventSeqs`](#persistence-type-sessioneventenvelope46sourceeventseqs) |
| `surfaceOp` | 可选 | [`SessionEventEnvelope[46].surfaceOp`](#persistence-type-sessioneventenvelope46surfaceop) |
| `time` | 必需 | `number` |
...
来源：`packages/core/session/src/types.ts:300`
...
来源：`packages/interaction/commands/src/types.ts:104`
...
来源：`packages/interaction/commands/src/types.ts:97`
...
来源：`packages/core/session/src/types.ts:62`
...
| `shadowedSeqs` | 必需 | [`SessionEventEnvelope[46].sourceEventSeqs`](#persistence-type-sessioneventenvelope46sourceeventseqs) |
...
来源：`packages/compaction/compaction/src/types.ts:38` · `packages/compaction/compaction/src/types.ts:84`
...
来源：`packages/feedback/command-feedback/src/index.ts:62`
...
来源：`packages/llm/llm-retry/src/types.ts:17`
...
来源：`packages/llm/llm-retry/src/types.ts:30`
...
来源：`packages/core/session/src/types.ts:329`
...
来源：`packages/session/session-log-deepseek/src/types.ts:57`
...
来源：`packages/session/session-title/src/types.ts:29`
...
来源：`packages/session/session-title/src/types.ts:30`
...
来源：`packages/core/session/src/types.ts:277` · `packages/core/session/src/types.ts:279`
...
来源：`packages/experimental/agent-team/src/types.ts:229`
...
来源：`packages/core/session/src/types.ts:306`
...
来源：`packages/core/session/src/types.ts:318`
...
来源：`packages/core/session/src/types.ts:322`
...
来源：`packages/util/values/src/index.ts:4`
...
来源：`packages/core/session/src/types.ts:275`
...
来源：`packages/core/session/src/types.ts:194`
...
来源：`packages/core/session/src/types.ts:196`
...
来源：`packages/core/session/src/types.ts:183`
...
来源：`packages/core/session/src/types.ts:184`
...
来源：`packages/core/session/src/types.ts:185`
...
来源：`packages/core/session/src/types.ts:188`
...
来源：`packages/core/session/src/types.ts:198`
...
来源：`packages/core/session/src/types.ts:204`
...
来源：`packages/core/session/src/types.ts:211`
...
来源：`packages/core/session/src/types.ts:266`
...
来源：`packages/web/web-search-deepseek/src/provider.ts:62`
...
来源：`packages/web/web-search-deepseek/src/provider.ts:65`
...
来源：`packages/llm/llm/src/types.ts:54` · `packages/web/web-search-deepseek/src/provider.ts:67`
...
来源：`packages/web/web-search-deepseek/src/provider.ts:72`
...
来源：`packages/core/session/src/types.ts:29`
...
来源：`packages/api/session-controller/src/types.ts:82`
...
来源：`packages/attachment/attachment/src/types.ts:11`
...
来源：`packages/attachment/attachment/src/types.ts:8`
...
来源：`packages/context/agent-instructions/src/render.ts:47`
...
来源：`packages/context/agent-instructions/src/state.ts:37`
...
来源：`packages/context/session-reference/src/types.ts:13`
...
来源：`packages/core/agent/src/types.ts:29`
...
来源：`packages/core/session/src/types.ts:222`
...
来源：`packages/core/session/src/types.ts:234`
...
来源：`packages/core/session/src/types.ts:251`
...
来源：`packages/core/session/src/types.ts:92`
...
来源：`packages/core/session/src/types.ts:215`
...
来源：`packages/core/tools/src/index.ts:673`

| 属性 | 存在性 | 类型 |
|---|---|---|
| `allow` | 可选 | [`event:request/header[0].data.header.config.stop`](#persistence-type-eventrequestheader0dataheaderconfigstop) |
| `deny` | 可选 | [`event:request/header[0].data.header.config.stop`](#persistence-type-eventrequestheader0dataheaderconfigstop) |
...
来源：`packages/core/tools/src/types.ts:20`
...
来源：`packages/core/tools/src/types.ts:11`
...
来源：`packages/experimental/agent-team/src/types.ts:44`
...
来源：`packages/experimental/agent-team/src/types.ts:47`
...
来源：`packages/experimental/agent-team/src/types.ts:106`

| 属性 | 存在性 | 类型 |
|---|---|---|
| `content` | 必需 | [`event:agent/inbox/spliced[0].data.inserted[0].content`](#persistence-type-eventagentinboxspliced0datainserted0content) |
| `delivery` | 必需 | [`event:team/message/queued[0].data.message.delivery`](#persistence-type-eventteammessagequeued0datamessagedelivery) |
| `id` | 必需 | `string` |
| `senderId` | 必需 | `string` |
...
来源：`packages/experimental/agent-team/src/types.ts:116`
...
来源：`packages/experimental/agent-team/src/types.ts:74`

| 属性 | 存在性 | 类型 |
|---|---|---|
| `blockedBy` | 必需 | [`event:request/header[0].data.header.config.stop`](#persistence-type-eventrequestheader0dataheaderconfigstop) |
| `description` | 必需 | `string` |
| `id` | 必需 | `string` |
| `ownerId` | 可选 | `string` |
| `revision` | 必需 | `number` |
...
| `subject` | 必需 | `string` |
| `writeScopes` | 必需 | [`event:request/header[0].data.header.config.stop`](#persistence-type-eventrequestheader0dataheaderconfigstop) |
...
来源：`packages/experimental/agent-team/src/types.ts:71`
...
来源：`packages/goal/goal/src/domain.ts:44`
...
来源：`packages/goal/goal/src/domain.ts:35`
...
来源：`packages/goal/goal/src/domain.ts:47`
...
来源：`packages/goal/goal/src/domain.ts:24`
...
来源：`packages/goal/goal/src/types.ts:51`
...
来源：`packages/goal/goal/src/types.ts:44`
...
来源：`packages/goal/goal/src/types.ts:19`
...
来源：`packages/goal/goal/src/types.ts:59`
...
来源：`packages/hooks/hook-protocol/src/types.ts:48`
...
来源：`packages/interaction/user-approval/src/index.ts:60`
...
来源：`packages/interaction/user-approval/src/types.ts:32`
...
来源：`packages/llm/llm-retry/src/types.ts:16`
...
来源：`packages/llm/llm-retry/src/types.ts:43`
...
来源：`packages/llm/llm/src/call-config.ts:23`
...
| `provider` | 必需 | `string` |
| `reasoningEffort` | 可选 | `string` |
| `stop` | 可选 | [`event:request/header[0].data.header.config.stop`](#persistence-type-eventrequestheader0dataheaderconfigstop) |
...
来源：`packages/llm/llm/src/call-config.ts:36`
...
来源：`packages/llm/llm/src/message.ts:148`
...
来源：`packages/llm/llm/src/message.ts:65`
...
来源：`packages/llm/llm/src/message.ts:131`
...
来源：`packages/llm/llm/src/message.ts:128`
...
来源：`packages/llm/llm/src/message.ts:24`
...
来源：`packages/llm/llm/src/message.ts:29`
...
来源：`packages/llm/llm/src/message.ts:154`
...
来源：`packages/llm/llm/src/message.ts:143`
...
来源：`packages/llm/llm/src/types.ts:110`
...
来源：`packages/llm/llm/src/types.ts:108`
...
来源：`packages/llm/llm/src/types.ts:125`
...
来源：`packages/llm/llm/src/types.ts:71`
...
来源：`packages/llm/llm/src/types.ts:40`
...
来源：`packages/llm/llm/src/types.ts:60`
...
来源：`packages/llm/llm/src/types.ts:342`

| 属性 | 存在性 | 类型 |
|---|---|---|
| `blocks` | 可选 | [`event:assistant/chunk[0].data.chunk[6].replayState.blocks`](#persistence-type-eventassistantchunk0datachunk6replaystateblocks) |
...
来源：`packages/llm/llm/src/types.ts:364`
...
来源：`packages/llm/llm/src/types.ts:135`
...
来源：`packages/llm/llm/src/types.ts:78`
...
来源：`packages/llm/llm/src/types.ts:88`
...
来源：`packages/llm/llm/src/types.ts:385`
...
来源：`packages/sandbox/sandbox/src/index.ts:29`
...
来源：`packages/schedule/schedule/src/types.ts:13`
...
来源：`packages/schedule/schedule/src/types.ts:27`
...
来源：`packages/schedule/schedule/src/types.ts:93`
...
来源：`packages/schedule/schedule/src/types.ts:39`
...
来源：`packages/schedule/schedule/src/types.ts:86`
...
来源：`packages/schedule/schedule/src/types.ts:105`
...
来源：`packages/schedule/schedule/src/types.ts:72`
...
来源：`packages/schedule/schedule/src/types.ts:79`
...
来源：`packages/schedule/schedule/src/types.ts:69`
...
来源：`packages/session/session-persistence-jsonl/src/format.ts:46`
...
来源：`packages/session/session-title-llm/src/index.ts:27`

| 属性 | 存在性 | 类型 |
|---|---|---|
| `maxTokens` | 必需 | `number` |
| `messageSeqs` | 必需 | [`SessionEventEnvelope[46].sourceEventSeqs`](#persistence-type-sessioneventenvelope46sourceeventseqs) |
...
来源：`packages/session/session-title/src/types.ts:41`
...
来源：`packages/session/session-title/src/types.ts:20` · `packages/subagent/tool-subagent/src/model-selection.ts:9`
...
来源：`packages/session/session-title/src/types.ts:28`
...
来源：`packages/skill/skill/src/index.ts:148`
...
来源：`packages/skill/tool-skill/src/index.ts:34`
...
来源：`packages/subagent/subagent/src/continuation.ts:59`
...
来源：`packages/subagent/subagent/src/continuation.ts:74`
...
来源：`packages/subagent/subagent/src/descriptor.ts:72`
...
来源：`packages/subagent/subagent/src/descriptor.ts:61`
...
来源：`packages/subagent/subagent/src/descriptor.ts:89`
...
来源：`packages/todo/tool-todo/src/types.ts:21`
...
来源：`packages/web/web-search-deepseek/src/provider.ts:56`
...
来源：`packages/workflow/tool-workflow/src/types.ts:29`
...
来源：`packages/workflow/tool-workflow/src/types.ts:20`
...
来源：`packages/workflow/tool-workflow/src/types.ts:36`
...
来源：`packages/workflow/tool-workflow/src/types.ts:14`
...
来源：`packages/workflow/workflow/src/types.ts:110`
...
来源：`packages/workflow/workflow/src/types.ts:63`
...
来源：`packages/core/session/src/types.ts:404`
...
来源：`packages/api/session-controller/src/types.ts:362` · `packages/attachment/attachment/src/brand.ts:6` · `packages/compaction/compaction/src/brand.ts:4` · `packages/core/session/src/types.ts:17` · `packages/experimental/agent-team/src/types.ts:20` · `packages/experimental/agent-team/src/types.ts:32` · `packages/experimental/agent-team/src/types.ts:8` · `packages/goal/goal/src/types.ts:16` · `packages/interaction/commands/src/brand.ts:20` · `packages/interaction/user-approval/src/types.ts:17` · `packages/llm/llm-retry/src/brand.ts:4` · `packages/llm/llm/src/brand.ts:16` · `packages/llm/llm/src/brand.ts:31` · `packages/llm/llm/src/brand.ts:43` · `packages/llm/llm/src/brand.ts:55` · `packages/schedule/schedule/src/types.ts:10` · `packages/session/session-title/src/index.ts:42` · `packages/session/session-title/src/types.ts:17` · `packages/webhook/webhook/src/brand.ts:12` · `packages/webhook/webhook/src/brand.ts:6` · `packages/webhook/webhook/src/brand.ts:9` · `packages/workflow/workflow/src/types.ts:13`
...
仅使用源码的提取器以 TypeScript 6.0.3 运行。重建时省略冗余的 `object` 交叉类型成员，并丢弃规范化后不可达的类型定义；保留已有的 `any` / `unknown`，不以不透明类型替代未解析的导入。V0 与 V2 根类型摘要与从预发布归档重建的对应完整状态一致。
