...
| `ctx.documentRenderController` | `core` | [`api-document-render-controller`](../packages/api/document-render-controller) | - | - | - | Authorizes Session file reads and returns rendered PDF bytes with the source identity. |
| `ctx.officeToPdf` | `core` | [`office-to-pdf`](../packages/document/office-to-pdf) | - | [`api-document-render-controller`](../packages/api/document-render-controller) | - | Authorized Office bytes are converted on the Host using the declared native target engine, or Node WASM when no native target is declared. |
