...
flowchart LR
  ...
  svc_profileRuntime["ctx.profileRuntime<br/>Launcher-owned profile reload coordination"]
  ...
  svc_profileRuntime --> pkg_plugin_manager
...
| `ctx.profileRuntime` | `core` | [`app-boot`](../packages/boot/app-boot) | - | [`plugin-manager`](../packages/boot/plugin-manager) | - | dsh launcher 提供 profile 位置和组合逻辑；重载调度由 dsh-hmr 负责。 |
