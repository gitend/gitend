...
flowchart LR
  ...
  svc_documentRenderController["ctx.documentRenderController<br/>Host Office preview Remote"]
  ...
  pkg_api_document_render_controller --> svc_documentRenderController
  ...
  svc_officeToPdf --> pkg_api_document_render_controller
...
| `ctx.documentRenderController` | `core` | [`api-document-render-controller`](../packages/api/document-render-controller) | - | - | - | 授权 Session 文件读取，并返回带源文件身份的渲染 PDF 字节。 |
| `ctx.officeToPdf` | `core` | [`office-to-pdf`](../packages/document/office-to-pdf) | - | [`api-document-render-controller`](../packages/api/document-render-controller) | - | 已授权的 Office 字节在宿主上使用已声明的原生目标引擎转换；未声明原生目标时使用 Node WASM。 |
