...
- **委派权限**会在第一次 await 前建立快照。Auto 或 Full access 父级会把匹配的 `permission/preset` 身份追加到进程内子级的日志，而普通子级组装路径仍复制显式沙箱覆盖，并把审批固定为 `never`；seed 后的身份会阻止 fork 中更早的同旋钮组合身份胜出。共享 Auto integration 因而会独立审查继承 Auto 的子级的每个原生调用与 PTC inner call：low 允许，medium 要求既有创建 prompt 或经过核验的 human／直接父级消息明确授权动作／目标／范围且不与 human 限制冲突，high 拒绝。它从 `parentSession` 与既有消息派生这份上下文，不增加父 call metadata、delegation provenance、review receipt 或 Session format 变化。进程外 ACP、DSH SDK、Codex 与 Claude Code 子级保留各自的权限系统；只有父级的委派工具调用接受审查。
