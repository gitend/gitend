...
- **委派权限**会在第一次 await 前建立快照。Auto 父级会把 `permission/preset:auto` 追加到进程内子级的日志，而普通子级组装路径仍复制显式沙箱覆盖，并把审批固定为 `never`。共享 Auto integration 因而会独立审查该子级的原生调用与 PTC inner call：low 允许，medium 必须符合既有创建 prompt、经过核验的直接父级消息与 durable human 限制，high 拒绝。它从 `parentSession` 与既有消息派生这份上下文，不增加父 call metadata、delegation provenance、review receipt 或 Session format 变化。进程外 ACP、DSH SDK、Codex 与 Claude Code 子级保留各自的权限系统；只有父级的委派工具调用接受审查。
