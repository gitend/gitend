...
PTC mode 会在策略之前记录每个实际开始的子分派，其中包含不可变的名称、描述、参数 schema 与规范化参数。随后，桥接层会把已结算子分派暴露给 `tools/ptc-dispatch-log` waterfall；它可以改变持久事件中的内容副本，但会保留程序取得的值、模型可见的外层结果与结构化失败身份：
...
结果仅承载产出。调用身份保留在不可变的 `ToolExecution` 上，后者伴随结果经过每个钩子，并出现在持久化的 `tool/call` / `tool/result` 会话事件上，因此包装层无法创建第二个相互矛盾的身份。规范的 `value` 仅存在于执行期间：循环只持久化 `content`、结构化 `error` 和 `meta`，`tool/code-dispatch` 则存储子调用渲染后的 `content`、`isError` 与可选结构化 `error`。回放可以重现展示，却无法重建规范的中间值。可选的 `ToolErrorInfo.reason` 是 raw user-facing detail，绝不会进入模型可见内容。
