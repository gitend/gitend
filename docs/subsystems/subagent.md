...
- **Delegated permission** snapshots before the first await. An Auto parent appends `permission/preset:auto` to an in-process child's log, and the ordinary child-composition path still copies an explicit sandbox override and pins approval to `never`. The shared Auto integration therefore reviews that child's native and PTC inner calls. Out-of-process ACP, DSH SDK, Codex, and Claude Code children keep their own permission systems; only the parent delegation tool call is reviewed.
