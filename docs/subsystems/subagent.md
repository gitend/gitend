...
- **Delegated permission** snapshots before the first await. An Auto or Full access parent appends the matching `permission/preset` identity to an in-process child's log, and the ordinary child-composition path still copies an explicit sandbox override and pins approval to `never`; the post-seed identity prevents a stale same-bundle fork value from winning. The shared Auto integration therefore reviews a child that inherited Auto independently on every native and PTC inner call: low is allowed, medium requires explicit action/target/scope authorization in the existing creation prompt or authenticated human/direct-parent messages without conflicting human limits, and high is denied. It derives this context from `parentSession` and existing messages without adding parent call metadata, delegation provenance, review receipts, or a Session-format change. Out-of-process ACP, DSH SDK, Codex, and Claude Code children keep their own permission systems; only the parent delegation tool call is reviewed.
