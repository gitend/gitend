...
The Web profile mounts [dsh-client-hmr](../../packages/client/hmr/README.md) to broadcast graph membership and rebuilt revisions through SSE. The browser adds and removes Loader entries from validated graphs; bundle stat polling supplies development rebuild notifications. The module host itself does not watch files.
