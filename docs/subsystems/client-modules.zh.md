...
Web profile 加载 [dsh-client-hmr](../../packages/client/hmr/README.zh.md)，通过 SSE 广播模块图成员变化和重建 revision。浏览器根据校验后的模块图增删 Loader 条目；bundle stat 轮询提供开发构建通知。模块宿主自身不监听文件。
