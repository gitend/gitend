...
Before changing the writer, archive its complete persistence schema and bilingual format reference under `docs/persistence-formats/vN.*`, preserving all earlier records. The [format coverage check](../persistence-formats/README.md#maintenance) requires every integer below the new writer to have its own document; the current generated catalog covers only the new writer.
