...
变更写入器之前，使用[归档命令](../persistence-formats/README.zh.md#maintenance)保留其完整持久化 schema，再在 `docs/persistence-formats/vN.*` 下添加双语格式参考。保留所有更早的记录。格式覆盖检查要求低于新写入器的每个整数版本都有独立文档；当前生成的目录只覆盖新写入器。
