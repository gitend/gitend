...
变更写入器之前，将其完整持久化 schema 与双语格式参考归档到 `docs/persistence-formats/vN.*`，并保留所有更早的记录。[格式覆盖检查](../persistence-formats/README.zh.md#maintenance)要求低于新写入器的每个整数版本都有独立文档；当前生成的目录只覆盖新写入器。
