...
it('records visible ordinary-update states without authorizing downloads or installation', async () => {
  ...
  const states = phases.map(phase => presentDesktopUpdate({ phase, version: '0.1.5-rc.2', percent: 58 }, zh))
...
it('keeps raw diagnostics out of error tooltips in both locales', () => {
  for (const messages of [en, zh]) {
    for (const failedOperation of ['check', 'download', 'install'] as const) {
      ...
      const presentation = presentDesktopUpdate(state, messages)
    ...
    expect(desktopUpdateErrorSummary({ phase: 'error', failedOperation: 'install', message: messages.updateStopFailed,
      technicalDetails: 'private diagnostic' }, messages)).toBe(messages.updateStopFailed)
    for (const message of [messages.updateTasksChanged, messages.updateTasksUnavailable]) {
      expect(desktopUpdateErrorSummary({ phase: 'error', failedOperation: 'install', message }, messages)).toBe(message)
    }
