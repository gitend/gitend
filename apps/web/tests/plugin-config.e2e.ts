...
describe('web e2e: plugin configuration section', () => {
  ...
  it('groups both Subagent namespaces into one card', async () => {
  ...
  it('saves subagent limits and resets them to the deployment defaults', async () => {
    ...
    await dialog.getByRole('button', { name: '展开设置: Subagent', exact: true }).waitFor()
    ...
    expect(await depth.inputValue()).toBe('3')
    expect(await capacity.inputValue()).toBe('8')
    await dialog.getByRole('button', { name: '收起设置: Subagent', exact: true }).click()
  })
  ...
  it('opens field explanations with the keyboard and retains unsaved edits', async () => {
    ...
    const depth = dialog.getByLabel('最大递归深度', { exact: true })
    await depth.fill('2')
    const depthHelp = dialog.getByRole('button', { name: '最大递归深度说明', exact: true })
    expect(await dialog.getByRole('region', { name: '最大递归深度说明', exact: true }).count()).toBe(0)
    await depthHelp.press('Enter')
    const depthRules = dialog.getByRole('region', { name: '最大递归深度说明', exact: true })
  ...
  it('saves limits and the model allowlist together from the shared card', async () => {
    ...
    await dialog.getByLabel('最大递归深度', { exact: true }).fill('2')
    await toggle.click()
  ...
  it.skipIf(MODE === 'record')('keeps the fixture inventory closed', async () => {
    ...
    await assertFixtureInventory(SNAPSHOT_DIR, ['section.expected.md', 'subagent.expected.md'])
  })
})
