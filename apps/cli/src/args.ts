...
export function parseDshArgs(argv: readonly string[], version: string): DshInvocation {
  ...
  const plugin = program.command('plugin').description('manage a profile\'s plugins: add and remove install through the plugin manager, every other verb is forwarded to pnpm in the profile directory')
  plugin
    .requiredOption('--profile <name>', 'the profile whose plugins to manage (initialized on first use)')
    .allowUnknownOption()
    .argument('[args...]', 'add <pkg...>, remove <pkg...>, or any pnpm arguments forwarded verbatim (why <pkg>, update, ...)')
