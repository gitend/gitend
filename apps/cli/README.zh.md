...
`@deepseek-ai/dsh/profile-boot` 仅导出 `runProfile`、`RunProfileOptions` 和 `ResolvedProfileRuntime`，向 Desktop Host 提供共享 profile 生命周期；启动器辅助函数保留在内部。已解析的应用 profile 指定自己的安装锚点和 profile 内模块补全，同时沿用 Harness home patch、代理环境、遥测开关、patch 热重载和有界关闭。
